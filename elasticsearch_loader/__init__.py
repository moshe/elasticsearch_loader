...
def cli(ctx, **opts):
    ...
    if opts['index_settings_file']:
        if ctx.obj['es_conn'].indices.exists(index=opts['index']):
            ctx.obj['es_conn'].indices.put_settings(index=opts['index'], body=json.load(opts['index_settings_file']))
        else:
            ctx.obj['es_conn'].indices.create(index=opts['index'], body=json.load(opts['index_settings_file']))
